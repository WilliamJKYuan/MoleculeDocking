...
collect_chimerax_inputs() {  # 获取 ChimeraX 分析所需参数；正式运行前一次性完成
    ...
    read -r -p "请输入 ChimeraX 并行实例数（1=单线程；最多不超过受体数；默认 1）: " CHIMERAX_PARALLEL_JOBS
    CHIMERAX_PARALLEL_JOBS=$(echo "${CHIMERAX_PARALLEL_JOBS:-1}" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
    if ! [[ "$CHIMERAX_PARALLEL_JOBS" =~ ^[0-9]+$ ]] || [ "$CHIMERAX_PARALLEL_JOBS" -lt 1 ]; then
        echo "请输入大于等于 1 的整数。"
        return 1
    fi
}
...
collect_all_inputs() {  # 所有需要询问的内容集中放在正式运行之前
    ...
    if [ "${need_chimerax:-0}" -eq 1 ]; then
        ...
        echo "ChimeraX并行实例数: ${CHIMERAX_PARALLEL_JOBS:-1}"
...
get_chimerax_receptor_names() {
    if [ -n "${CHIMERAX_RECEPTORS:-}" ]; then
        echo "$CHIMERAX_RECEPTORS" | tr ',;' '  ' | tr ' ' '\n' | sed '/^[[:space:]]*$/d'
        return 0
    fi
    shopt -s nullglob
    local f name
    for f in "$RECEPTOR_DIR_ABS"/*.pdbqt; do
...
merge_chimerax_parallel_outputs() {
    local partial_dir="$1"
    local final_txt="$2"
    local py_script="$WORK_DIR_ABS/_merge_chimerax_parallel_outputs_temp.py"
    local partial_dir_py final_txt_py work_dir_py receptor_dir_py docking_dir_py
    partial_dir_py=$(to_native_path "$partial_dir")
    final_txt_py=$(to_native_path "$final_txt")
    work_dir_py=$(to_native_path "$WORK_DIR_ABS")
    receptor_dir_py=$(to_native_path "$RECEPTOR_DIR_ABS")
    docking_dir_py=$(to_native_path "$DOCK_OUT_DIR_ABS")
...
partial_dir = Path(r'''$partial_dir_py''')
final_txt = Path(r'''$final_txt_py''')
work_dir = r'''$work_dir_py'''
receptor_dir = r'''$receptor_dir_py'''
docking_dir = r'''$docking_dir_py'''

def section_lines(text, start_marker, next_markers):
    ...
    for line in lines:
        if line.strip() == start_marker:
            active = True
            continue
        if active and any(line.strip() == m for m in next_markers):
            break
        if active:
            out.append(line)
...
def data_without_header(rows):
    if not rows:
        return []
    return [r for r in rows[1:] if r.strip()]

files = sorted(partial_dir.glob("*.txt"))
if not files:
    print(f"[ERROR] No partial ChimeraX TXT files found in {partial_dir}", file=sys.stderr)
    sys.exit(1)
...
for f in files:
    ...
    selected = section_lines(
        text,
        "===== SELECTED RESULT PER RECEPTOR-LIGAND PAIR =====",
        ["===== ALL POSE DETAILS =====", "===== RAW HBOND LINES PARSED FROM CHIMERAX SAVEFILE ====="],
    )
    poses = section_lines(
        text,
        "===== ALL POSE DETAILS =====",
    ...
    selected_rows.extend(data_without_header(selected))
    pose_rows.extend(data_without_header(poses))
    if raw:
        raw_blocks.append(f"\\n--- PARTIAL FILE: {f.name} ---")
        raw_blocks.extend(raw)

def row_key(row):
    parts = row.split("\\t")
    rec = parts[0] if len(parts) > 0 else ""
    lig = parts[1] if len(parts) > 1 else ""
    pose = parts[2] if len(parts) > 2 else ""
    return (rec.lower(), lig.lower(), pose)

selected_rows = sorted(selected_rows, key=row_key)
pose_rows = sorted(pose_rows, key=row_key)

final_txt.parent.mkdir(parents=True, exist_ok=True)
lines = []
lines.append("ChimeraX Vina H-bond batch summary\\n")
lines.append("Merged from parallel ChimeraX instances\\n")
lines.append(f"WORK_DIR: {work_dir}\\n")
lines.append(f"RECEPTOR_DIR: {receptor_dir}\\n")
lines.append(f"DOCKING_DIR: {docking_dir}\\n")
lines.append(f"Partial files merged: {len(files)}\\n")
lines.append(f"Total receptor-ligand pairs: {len(selected_rows)}\\n")
lines.append(f"Total poses analyzed: {len(pose_rows)}\\n")
lines.append(f"Relax H-bond criteria: {r'''${CHIMERAX_RELAX_HBOND:-1}'''}\\n")
lines.append("\\n")
...
final_txt.write_text("".join(lines), encoding="utf-8")
print(f"[OK] Merged {len(files)} partial ChimeraX result files -> {final_txt}")
PYEOF

    local python_cmd exit_code
    python_cmd=$(command -v python 2>/dev/null || command -v python3 2>/dev/null || command -v py 2>/dev/null || true)
    [ -z "$python_cmd" ] && { echo "错误: 找不到 Python，无法合并 ChimeraX 并行结果。"; rm -f "$py_script"; return 1; }
    if [[ "$(basename "$python_cmd")" = "py" || "$(basename "$python_cmd")" = "py.exe" ]]; then
        "$python_cmd" -3 "$py_script"
    else
        "$python_cmd" "$py_script"
...
step_chimerax_analysis() {  # Step 4: 直接读取 receptor 目录与 Vina 输出目录，运行 ChimeraX 氢键分析
    ...
    local cmd_to_run script_for_chimerax parallel_jobs
    cmd_to_run="$CHIMERAX_CMD"
    script_for_chimerax=$(to_native_path "$CHIMERAX_SCRIPT_ABS")
    parallel_jobs="${CHIMERAX_PARALLEL_JOBS:-1}"
    ...
    while IFS= read -r rec; do
        [ -n "$rec" ] && receptors+=("$rec")
    done < <(get_chimerax_receptor_names)
    if [ "${#receptors[@]}" -eq 0 ]; then
        echo "错误: 未找到可用于 ChimeraX 分析的受体。"
        return 1
    fi
    [ "$parallel_jobs" -gt "${#receptors[@]}" ] && parallel_jobs="${#receptors[@]}"

    local partial_dir part_idx i receptors_csv output_txt_native pid failed
    partial_dir="$DOCK_OUT_DIR_ABS/chimerax_parallel_parts_$$"
    mkdir -p "$partial_dir" || return 1

    echo "将 ${#receptors[@]} 个受体分配到 $parallel_jobs 个 ChimeraX 实例。"
    for ((part_idx=0; part_idx<parallel_jobs; part_idx++)); do
        local group=()
        for ((i=part_idx; i<${#receptors[@]}; i+=parallel_jobs)); do
            group+=("${receptors[$i]}")
        done
        [ "${#group[@]}" -eq 0 ] && continue
        receptors_csv=$(join_by_comma "${group[@]}")
        output_txt_native=$(to_native_path "$partial_dir/chimerax_part_$((part_idx + 1)).txt")
        echo "    实例 $((part_idx + 1))/$parallel_jobs: $receptors_csv"
        (
            export CHIMERAX_RECEPTORS="$receptors_csv"
            export CHIMERAX_OUTPUT_TXT="$output_txt_native"
            "$cmd_to_run" --nogui --exit --script "$script_for_chimerax"
        ) &
        pid=$!
        echo "$pid" >> "$partial_dir/_pids.txt"
    done
    ...
    done < "$partial_dir/_pids.txt"
    if [ "$failed" -ne 0 ]; then
        echo "错误: 至少一个 ChimeraX 并行实例失败。部分结果目录: $partial_dir"
        return 1
    fi

    merge_chimerax_parallel_outputs "$partial_dir" "$CHIMERAX_OUTPUT_TXT_ABS" || return 1
    echo "ChimeraX并行分析完成: $CHIMERAX_OUTPUT_TXT_ABS"
    echo "分实例结果保留在: $partial_dir"
}
