...
collect_mgltools() {  # 只在参数收集阶段获取 MGLTools 根目录、prepare_ligand4.py 和自带 Python
    ...
    if [ -n "$MGLTOOLS_DIR" ]; then
        ...
        echo "未识别 MGLTools，请输入安装目录，例如 C:\\Program Files (x86)\\MGLTools-1.5.7"
...
detect_chimerax_cmd() {  # 自动查找 Windows/Linux 下的 ChimeraX 可执行文件
    ...
    # 2) 常见安装位置 + 版本号目录，例如 ChimeraX 1.12
    if [ "$SYSTEM_TYPE" = "Windows" ]; then
        local drives=(/c /d /e)
...
collect_all_inputs() {  # 所有需要询问的内容集中放在正式运行之前
    ...
    if [ "$can_run_chimerax_after" -eq 1 ]; then
        ...
        if [ "$RUN_CHIMERAX_AFTER" -eq 1 ]; then
            need_chimerax=1
            ask_yes_no_var RUN_PYMOL_FINAL_AFTER "ChimeraX 氢键分析完成后，继续运行 PyMOL 最终可视化？[y/N]（默认N）: " "n" || return 1
    ...
    if [ "$need_pymol_sdf" -eq 1 ]; then
        echo "PyMOL SDF→PDB 运行模式：1) GUI（默认）  2) No-GUI"
        read -r -p "选择 PyMOL SDF→PDB 运行模式 [默认: 1]: " PYMOL_MODE
...
collect_pymol_final_inputs() {  # 获取 PyMOL 最终可视化参数；输出默认放入工作目录/Final
    ...
    read -r -p "PyMOL最终图片/PSE输出目录 [默认: 工作目录/Final]: " input
    input=$(normalize_path "$input")
    if [ -z "$input" ]; then
    ...
    read -r -p "PyMOL可视化受体名称（多个用逗号分隔；留空读取TXT中的全部结果）: " PYMOL_FINAL_RECEPTORS
    PYMOL_FINAL_RECEPTORS=$(echo "$PYMOL_FINAL_RECEPTORS" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')

    echo "PyMOL最终输出内容：1) 仅输出PSE文件  2) 仅渲染图像  3) 输出PSE文件和渲染图像（默认）"
    read -r -p "选择 PyMOL 最终输出内容 [默认: 3]: " PYMOL_FINAL_OUTPUT_CHOICE
    ...
    case "$PYMOL_FINAL_OUTPUT_CHOICE" in
        ...
        3|both|Both|BOTH|all|All|ALL)
            ...
            ;;
        *)
            echo "错误: PyMOL最终输出内容只能输入 1/pse、2/image 或 3/both"
            return 1
            ;;
    esac

    if [ "$PYMOL_FINAL_RENDER_IMAGES" -eq 1 ]; then
        echo "PyMOL最终渲染方式：1) ray（默认，质量更高）  2) draw（速度更快，自动禁用GUI）"
        read -r -p "选择 PyMOL 最终渲染方式 [默认: 1]: " PYMOL_FINAL_RENDER_CHOICE
    ...
    if [ "$PYMOL_FINAL_RENDER_ENGINE" = "draw" ]; then
        PYMOL_FINAL_MODE=2
        echo "已选择 draw 渲染：自动使用 No-GUI 模式，不再启用 GUI。"
    else
        echo "PyMOL最终可视化运行模式：1) GUI  2) No-GUI（默认）"
        read -r -p "选择 PyMOL 最终可视化运行模式 [默认: 2]: " PYMOL_FINAL_MODE
