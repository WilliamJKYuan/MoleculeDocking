...
collect_pymol_final_inputs() {  # 获取 PyMOL 最终可视化参数；输出默认放入工作目录/Final
    ...
    echo "PyMOL输出：1) 仅输出PSE文件  2) 仅渲染图像  3) 输出PSE文件和渲染图像（默认）"
    read -r -p "选择 PyMOL 输出 [默认: 3]: " PYMOL_FINAL_OUTPUT_CHOICE
    ...
    if [ "$PYMOL_FINAL_RENDER_IMAGES" -eq 1 ]; then
        echo "PyMOL最终渲染方式：1) ray（默认，质量更高）  2) draw（速度更快，禁用GUI）"
    ...
    if [ "$PYMOL_FINAL_RENDER_ENGINE" = "draw" ]; then
        ...
        echo "PyMOL运行模式：1) GUI  2) No-GUI（默认）"
        read -r -p "选择 PyMOL 运行模式 [默认: 2]: " PYMOL_FINAL_MODE
